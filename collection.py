...
class Collection:
	...
	def getItemsAll(self, id, cat, maxN=500):
		...
		rst=[{'id':item.id,'title':item.title, 'status':item.db_status, 'ctm':item.updated} for item in feeds.entries]
		maxN=min(int(feeds.feed.opensearch_totalresults), maxN)
		start=51
		while start<maxN:
			feeds=self.getItems(id, cat, start)
			if feeds is not None: rst.extend([{'id':item.id, 'title':item.title, 'status':item.db_status, 'ctm':item.updated} for item in feeds.entries])
			start+=50
		return rst
...
def test2():
	...
	rst=client.getMoviesAll(1000001)
